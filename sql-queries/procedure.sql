...
create or replace procedure cancel_booking(bookingid int,cancel_time timestamp without time zone)
language plpgsql
as $$
declare
...
reg_no varchar(50);
begin
	select start_time into start_date from booking where booking_id = bookingid;
	if (start_date::date - cancel_time::date >=2) then
	begin
	select registration_no into reg_no from booking where booking_id = bookingid;
	update booking
	set cancelled_status = 'YES'
...
create or replace procedure cancel_pooling(poolingid int,cancel_time timestamp without time zone)
language plpgsql
as $$
declare bookingid int;
declare start_date timestamp without time zone;
begin
	select booking_id into bookingid from pooling where pooling_id = poolingid;
	select start_time into start_date where booking_id = bookingid;
	if (start_date::date - cancel_time::date >= 1) then
	begin
    update pooling
	set cancelled_status = 'YES'
	where pooling_id = poolingid;
	end;
	end if;
