...
router.post('/',async (req,res,next)=>{
    ...
    // getting expiry date
    q2 = `select user_id,expiry_date from membership natural join customer_info natural join login_info where expiry_date >= now()::date and user_name = '${req.session.username}'` 
    try {
        const result = await new Promise((resolve, reject) => {
            dbConnect.query(q2, (err, result) => {
                if (err) {
